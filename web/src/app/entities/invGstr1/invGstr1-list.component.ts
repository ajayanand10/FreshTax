...
import { InvGstr1DetailComponent } from './invGstr1-detail.component';
import { InvGstr1Service } from './invGstr1.service';

@Component({
    moduleId: module.id,
	templateUrl: 'invGstr1-list.component.html',
	selector: 'invGstr1-list'
})
export class InvGstr1ListComponent {

    @Input() header = "InvGstr1s...";
    ...
    invGstr1ToDelete : InvGstr1;

    // basic search criterias (visible if not in 'sub' mode)
    example : InvGstr1 = new InvGstr1();

    // list is paginated
    currentPage : PageResponse<InvGstr1> = new PageResponse<InvGstr1>(0,0,[]);


    constructor(private router : Router,
        private invGstr1Service : InvGstr1Service,
    ...
    loadPage(event : LazyLoadEvent) {
        this.invGstr1Service.getPage(this.example, event).
            subscribe(
    ...
    onRowSelect(event : any) {
        ...
        this.router.navigate(['/invGstr1', id]);
    }
    ...
    addNew() {
        if (this.sub) {
            ...
            this.router.navigate(['/invGstr1', 'new']);
        }
    }

    showDeleteDialog(rowData : any) {
        let invGstr1ToDelete : InvGstr1 = <InvGstr1> rowData;

        let dialogRef = this.confirmDeleteDialog.open(ConfirmDeleteDialogComponent);
        dialogRef.afterClosed().subscribe(result => {
            if (result === 'delete') {
                this.delete(invGstr1ToDelete);
            }
        });
    }

    private delete(invGstr1ToDelete : InvGstr1) {
        let id =  invGstr1ToDelete.id;

        this.invGstr1Service.delete(id).
            subscribe(
                response => {
                    this.currentPage.remove(invGstr1ToDelete);
